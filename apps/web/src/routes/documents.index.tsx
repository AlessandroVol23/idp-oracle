...
const searchSchema = z.object({
  docType: z.enum(['invoice', 'contract', 'cv', 'unknown']).optional(),
  status: z
    .enum([
      'pending',
      'text_extracted',
            ...
            <tbody>
              {items.map((doc) => (
                <tr key={doc.id} className="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3"><Badge tone="type">{doc.docType}</Badge></td>
                  <td className="px-4 py-3"><Badge tone="status">{doc.status}</Badge></td>
                  <td className="px-4 py-3 text-slate-600">{(doc.byteSize / 1024).toFixed(1)} KB</td>
