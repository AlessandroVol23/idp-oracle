...
async function listFiles(): Promise<string[]> {
  const out: string[] = [];
  for (const subdir of ['invoices', 'contracts', 'cvs']) {
...
async function main() {
  ...
  for (const r of results) {
    ...
    console.log(`  ${flag} ${r.filename}  type=${r.docType}  status=${r.status}  ${(r.durationMs / 1000).toFixed(1)}s${r.failedReason ? `  reason=${r.failedReason}` : ''}`);
  }
