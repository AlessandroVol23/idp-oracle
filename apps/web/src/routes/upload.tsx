...
function UploadPage() {
  ...
  const uploadMutation = useMutation({
    ...
    onError: (err, file) => {
      setEntries((prev) =>
        prev.map((e) => (e.filename === file.name && !e.documentId ? { ...e, error: String(err) } : e)),
      );
    },
  });
  ...
  return (
    <>
      ...
      >
        ...
        <p className="mt-4 text-xs text-slate-500">PDF only, up to {MAX_UPLOAD_BYTES / 1024 / 1024} MB.</p>
      </div>
    ...
    <Card>
      <div className="flex items-center justify-between">
        <div>
          ...
          {data?.failedReason && (
            <div className="text-xs text-red-700">{data.failedReason}</div>
          )}
        </div>
